...
def dct_2d(x):
    """Compute 2D DCT-II using FFT-based method"""
    N, M = x.shape[-2], x.shape[-1]
    
    # Create even-symmetric extension
    x_ext = torch.cat([x, x.flip(-2)], dim=-2)  # Extend along height
    x_ext = torch.cat([x_ext, x_ext.flip(-1)], dim=-1)  # Extend along width

    # Compute FFT (No fftshift needed)
    X = torch.fft.fft2(x_ext)

    # Take real part and keep first N×M values
    return X.real[..., :N, :M]

def idct_2d(X):
    """Compute 2D Inverse DCT-II using FFT-based method"""
    N, M = X.shape[-2], X.shape[-1]

    # Create even-symmetric extension
    X_ext = torch.cat([X, X.flip(-2)], dim=-2)
    X_ext = torch.cat([X_ext, X_ext.flip(-1)], dim=-1)

    # Compute IFFT (No ifftshift needed)
    x_reconstructed = torch.fft.ifft2(X_ext).real

    return x_reconstructed[..., :N, :M]

def visualize_fourier(gt_patch):
    """Visualizes the magnitude spectrum of a Fourier-transformed tensor."""
    magnitude_spectrum = torch.abs(gt_patch)
...
def get_dataset(args):
    if 'laion' in args.dataset:
        ...
        table = pq.read_table('eval.parquet')
...
def get_watermarking_pattern(pipe, args, device,w_pattern, shape=None):
    ...
    elif 'pizza' in w_pattern:
        size = gt_init.shape[-1]
        x, y = np.meshgrid(np.linspace(-1, 1, size), np.linspace(-1, 1, size))
        r = np.round(np.sqrt(x**2 + y**2), 3)  # Create radius values

        mean, stddev = 0, 1  # Define reasonable defaults
        radial_values = {val: np.random.normal(loc=mean, scale=stddev) for val in np.unique(r)}
        constant_radial_image = np.vectorize(lambda x: radial_values.get(x, 0))(r)
        
        # Convert to PyTorch tensor and reshape to match gt_init
        gt_patch = torch.tensor(constant_radial_image, device=device, dtype=gt_init.dtype)
        gt_patch = gt_patch.expand_as(gt_init)
    
    return gt_patch


def inject_watermark(init_latents_w, watermarking_mask, gt_patch, args,w_injection):
    init_latents_w_fft = torch.fft.fftshift(torch.fft.fft2(init_latents_w), dim=(-1, -2))
    if w_injection == 'complex':
        init_latents_w_fft[watermarking_mask] = gt_patch[watermarking_mask].clone()
    elif w_injection == 'seed':
        init_latents_w[watermarking_mask] = gt_patch[watermarking_mask]
